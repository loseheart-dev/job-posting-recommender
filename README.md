...
- 支持网站配置、采集任务调度、岗位筛选和结果展示。采用按需或定时批量采集，不承诺持续更新。

## 分工

卢世豪负责采集、解析、清洗；丁伟哲负责求职者画像和推荐；郑维豪负责主题分析与模型；徐挚凌负责后端服务与调度；王宇杰负责前端和可视化。团队按接口独立开发，审核后合并。
...
- 薪资口径复算：`python -m tests.verify_salary_real_data`；结果见 `tests/fixtures/real_data_salary_verification.json`。
