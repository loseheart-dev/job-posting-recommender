...
- [x] R6 数据可视化、职位展示和个性化推荐页面（王宇杰）
- [x] R7 采集、清洗、分析、推荐、展示完整链路
...
- [x] Z7 与王宇杰页面集中联调 + README 启动说明（README 启动说明与联调契约见 PR #30；页面五模块 AppTest 联调验证脚本 `tests/verify_app_integration.py`，服务 API 与页面调用一致，无异常）
...
- [x] T8 D4 用 8,608 条真实数据 fixture 完成三画像推荐验证与联调格式确认；证据成文 docs/25051415/ 并随 PR #20 提交（025b08d）
- [x] T9 D5 修正 D1.md 为 9/1 真实记录（PR #45 已合并）；执行组长 PR #49/#51 second review 并 APPROVE 留痕（5121251826/5121252506）；D5 记录导出随 PR #52

## 王宇杰任务

- [x] 完成 GitHub 认证与仓库协作者权限确认（gundame）
- [x] 创建个人分支 feat/wangyujie-frontend
- [x] 完成 Streamlit 页面骨架与五个导航模块
- [x] 市场概览：KPI 卡片、高频技能、城市分布
- [x] 岗位检索：筛选 + CSV 下载 + 岗位详情区
- [x] 分析洞察：三列布局、薪资分布、薪资因素、能力图谱、企业画像、岗位聚类、薪资预测
- [x] 个性化推荐：画像输入 + basic/multifactor 双模式 + st.session_state 保存
- [x] 采集管理：站点配置增删改查、启停、手动触发任务、任务记录
- [x] 视觉规范：背景、文字、卡片边框、表单面板、语义色彩
- [x] 接入 analysis_service 服务层
- [x] PR #29 已合并到 main
- [x] D1 日报和提示词记录已提交
- [x] D2 日报和提示词记录待提交
- [x] 补充页面交互验收截图与规范对应说明
...
- [x] M7 压缩 D2、D3 日报并将 800 字限制写入 `AGENTS.md`
- [x] M8 创建 D4 日报、提示词和 fixtures 可复现统计文件
...
- [x] 完成王宇杰页面和可视化（见"王宇杰任务"卡）
- [x] 接入真实清洗数据，完成集中联调和演示
- [x] 每日补充本人的日报和提示词导出
